...
use crate::dashboard::Notification;
...
type Clients = Arc<RwLock<Vec<broadcast::Sender<WebSocketMessage>>>>;
...
async fn handle_socket(socket: WebSocket, state: Arc<TradingState>) {
    ...
    clients.write().await.push(tx.clone());
    ...
    // Handle incoming messages (keep-alive ping/pong)
...
pub async fn broadcast_message(
    ...
    // The state holds the broadcast channel
    if let Some(broadcaster) = state.get_broadcaster().await {
        broadcaster.send(message)?;
    }
    Ok(())
}

/// Extension trait for TradingState to add broadcaster support
trait Broadcaster {
    async fn get_broadcaster(&self) -> Option<broadcast::Sender<WebSocketMessage>>;
    async fn set_broadcaster(&self, sender: broadcast::Sender<WebSocketMessage>);
}

impl TradingState {
    /// Create price update message
    pub fn create_price_update(market: String, price: f64, change_rate: f64) -> WebSocketMessage {
        use chrono::Utc;
    ...
    pub fn create_notification(notification_type: String, message: String) -> WebSocketMessage {
        use chrono::Utc;
    ...
    pub fn create_system_status(status: String, websocket_connected: bool) -> WebSocketMessage {
        use chrono::Utc;
...
impl WebSocketBroadcaster {
    ...
    pub fn broadcast_price(&self, market: String, price: f64, change_rate: f64) {
        let msg = TradingState::create_price_update(market, price, change_rate);
    ...
    pub fn broadcast_trade(&self, market: String, trade_type: String, price: f64, amount: f64) {
        let msg = TradingState::create_trade_update(market, trade_type, price, amount);
        let _ = self.broadcast(msg);
    }
...
mod tests {
    ...
    fn test_create_price_update() {
        let msg = TradingState::create_price_update("KRW-BTC".to_string(), 50000000.0, 0.05);
        match msg {
    ...
    fn test_create_trade_update() {
        let msg = TradingState::create_trade_update("KRW-BTC".to_string(), "buy".to_string(), 50000000.0, 0.001);
        match msg {
